...
rho.bounds <- function(p, R=diag(length(p))) {
  ...
  m = dim(R)[2]
  if (n!=m || R!=t(R)) {
    stop("Correlation matrix has to be symmetric!")
    break
  }
  q = 1 - p
  U = matrix(NA, n, n)
...
##'     R[i,j] = 0.90^abs(i-j)
##' }}
##' X0 <- start.matrix(p=c(0.4, 0.2, 0.5, 0.15, 0.4, 0.35, 0.2, 0.25, 0.3, 0.4), k = 5000)
##' X <- iter.matrix(X0, R=R1, T=5000, e.min = 1e-04)
##' GA.diagplot(R1, Rt=X$Rt, col.method="trafficlight")
##' GA.diagplot(R1, Rt=X$Rt, col.method="updown")
GA.diagplot <- function(R, Rt, eps=0.05, col.method="trafficlight", color=c(0, 8), top="") {
